...
contract AnyswapV6Router {
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            ISushiswapV2Pair(SushiswapV2Library.pairFor(factory, input, output)).swap(amount0Out, amount1Out, to, new bytes(0));
        }
    }
    ...
    function anySwapInExactTokensForTokens(
        ...
        uint256 deadline,
        uint256 fromChainID
    ) external virtual onlyMPC swapTradeEnabled ensure(deadline) returns (uint256[] memory amounts) {
